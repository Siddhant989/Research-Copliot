...
# Ordered fast-to-stable: newer models first, widely-available older ones as fallbacks.
# gemini-2.5-flash-lite is skipped — not a stable public model name.
DIRECT_MODELS    = ["gemini-2.5-flash", "gemini-2.0-flash", "gemini-1.5-flash"]
_DIRECT_BASE_URL = (
...
def gemini_direct_call(available_keys, exhausted_keys, parts,
    ...
    for api_key in list(available):
        key_dead      = False
        model_403_cnt = 0   # count how many models returned 403 for this key
        for model in DIRECT_MODELS:
            ...
            for attempt in range(1, _DIRECT_MAX_RETRIES + 1):
                ...
                try:
                    ...
                    # Permission denied (403): might be model-tier restriction, not key issue.
                    # Try the next model; if ALL models return 403 the key itself is dead.
        ...
        # If every model returned 403 for this key, the key itself is invalid/unauthorised
        if not key_dead and model_403_cnt >= len(DIRECT_MODELS):
            if api_key in available:
                available.remove(api_key)
                exhausted.append(api_key)
...
def save_api_key_to_env(api_key, env_path=None):
    ...
    key_name = f"GEMINI_API_KEY_{existing + 1}"
